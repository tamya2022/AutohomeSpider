...
class SliderValidator:
    def __init__(
            self,
            debug: bool = False,
            path_full_img: str = None,
            path_notch_img: str = None,
            action_name: str = "SliderValidator",
    ):
        self.debug = debug
        self.full_img_path = (
            path_full_img if path_full_img else f"full_img_path_{time.time()}.png"
        )
        self.notch_img_path = (
            path_notch_img if path_notch_img else f"notch_img_path_{time.time()}.png"
        )
    ...
    def operator_sport_v1(self, phys_params) -> tuple:
        ...
        # 物理算子当前所在的一维空间位置
        current_coordinate = (
            phys_params.get("current_coordinate")
            if phys_params.get("current_coordinate")
            else 0
        )
        # 切割变加速度的边界距离
        mid = phys_params.get("mid") if phys_params.get("mid") else boundary * 3.2 / 4
        # 运动时间（采样间隔）越高则单步位移提升越大，任务耗时降低，但误差增大
        t = phys_params.get("t") if phys_params.get("t") else 1.0
        # 运动初速度为0
        v = 0
    ...
    def identify_boundary(self) -> int:
        """获取缺口偏移量"""
        # 1.读取完整背景图与残缺背景图
        # 完整背景图与残缺背景图的边长参数一致
        full_image = Image.open(self.full_img_path)
        notch_image = Image.open(self.notch_img_path)
        ...
        for i in range(full_image.size[0]):
            ...
            for j in range(full_image.size[1]):
                ...
                if not self.is_pixel_equal(full_image, notch_image, i, j):
    ...
    def check_boundary(self, boundary):
        ...
        # 标识边界线（计算值）
        draw.line(
            (boundary, 0, boundary, boundary_notch.size[1]),
            fill=(30, 255, 12),
            width=line_width,
        )
        # 标识边界线x轴坐标
    ...
    def drag_slider(
        ...
        if is_hold:
            ...
            time.sleep(0.5)
            ActionChains(ctx).click_and_hold(slider).perform()
        # 震荡收敛步伐初始化
        catwalk = []
        # 参数表
        ...
        for step in track:
            ActionChains(ctx).move_by_offset(xoffset=step, yoffset=0).perform()
        ...
        if use_imitate:
            ...
            if position == 0:
                ...
                if abs(sum(catwalk)) >= int(step_num / 2):
                    ActionChains(ctx).move_by_offset(
                        xoffset=-sum(catwalk) + 1, yoffset=0
                    ).perform()
            else:
                if position > 0:
                    # 拼图位于boundary右方 -> 回落
                    # 修正后落于区间 ∈ [-2,1,2,3,4,5,6]
                    emergency_braking = -position
                else:
                    # 拼图位于boundary左方 -> 补偿
                    # 修正后落于区间 ∈ [3, 4, 5, 6, 7...]
                    emergency_braking = abs(position) - 7
                ...
                # 一级步态修正
                ActionChains(ctx).move_by_offset(
                    xoffset=emergency_braking, yoffset=0
                ).perform()
        ...
        if momentum_convergence:
            ...
            ActionChains(ctx).move_by_offset(xoffset=inertial, yoffset=0).perform()
            debugger_map.update({"inertial": inertial})
        # 打印参数表
        if self.debug:
            print(f"{self.action_name}: {debugger_map}")
        # 松开滑块 统计通过率
        ActionChains(ctx).release(slider).perform()
        return debugger_map

    def run(self, ctx: Chrome):
        """
